...
TranslateGemma is a CLI tool. There is no API service in this repository.
...
|   |   `-- translator.py     # TranslateGemma engine (llama.cpp)
|   |-- services/
...
3. `TranslateGemmaEngine` loads the GGUF model on first use and runs inference.
